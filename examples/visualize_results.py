...
Generate visualizations from experimental results CSV.
"""
import pandas as pd
...
def plot_model_comparison(results_df, output_dir='../results'):
    ...
    for i, dataset in enumerate(datasets):
        ...
        means = [data[data['model'] == m]['roc_auc_mean'].values[0] for m in models]
        stds = [data[data['model'] == m]['roc_auc_std'].values[0] for m in models]
        
        offset = width * (i - 0.5)
        ax.bar([xi + offset for xi in x], means, width, 
               yerr=stds, label=dataset, alpha=0.8, capsize=5)
    ...
    for i, dataset in enumerate(datasets):
        data = results_df[results_df['dataset'] == dataset]
        times = [data[data['model'] == m]['train_time_mean'].values[0] for m in models]
        
        offset = width * (i - 0.5)
        ax.bar([xi + offset for xi in x], times, width, 
    ...
    for dataset in datasets:
        data = results_df[results_df['dataset'] == dataset]
        ax.scatter(data['params'], data['roc_auc_mean'], 
                   s=100, alpha=0.7, label=dataset)
        
        for _, row in data.iterrows():
            ax.annotate(row['model'], 
                       (row['params'], row['roc_auc_mean']),
...
def print_summary_table(results_df):
    ...
    for dataset in results_df['dataset'].unique():
        ...
        print(f"{'Model':<25} {'ROC-AUC':<20} {'F1-Score':<20} {'Time/Epoch':<15}")
        print("-"*80)
        
        for _, row in data.iterrows():
            auc_str = f"{row['roc_auc_mean']:.4f} ± {row['roc_auc_std']:.4f}"
            f1_str = f"{row['f1_mean']:.4f} ± {row['f1_std']:.4f}"
            time_str = f"{row['train_time_mean']:.2f}s"
            
            print(f"{row['model']:<25} {auc_str:<20} {f1_str:<20} {time_str:<15}")
